...
API_KEY = 'ByN9RAlQW4OcstU3ZGkFz3qx'

SECRET_KEY = 'NyOEX1GH50whYCxwvHhn8wu4w62NBM7b'
...
def get_veggie(photo):
    ...
    # 菜品图1
    veggie = get_result(photo, url)
    return veggie


def get_details(veggie):
    content = '''1.木瓜
...
10.萝卜
营养成分：维生素C、钙、蛋白质、多种氨基酸、葡萄糖、脂肪、果糖、多种酶类、挥发油。 食疗作用：萝卜含有促进新陈代谢的物质，有助消除皮下脂肪，故有减肥作用。萝卜中的挥发油和芥子油可刺激肠胃蠕动，帮助消化，能增强食欲。萝卜中除了有降血压和防治胆石形成的作用外，更有抗癌的作用。
注意事项：腹泻、消化不良患者不宜多食。
...
81.芋头
营养成分：蛋白质、脂肪、维生素B1、维生素B2、维生素C、钙、磷、铁、碳水化合物、胡萝卜素、皂素。
食疗作用：芋头性平味甘辛，有宽肠胃、区烦热、补气益肾等功效，有助于防治久痢便血、口干等症状。可使皮肤白嫩，富有弹性。
注意事项：味酸过多、肠胃炎患者不宜多吃。
'''
    if veggie not in content:
        return 'To be completed', 'To be completed', 'To be completed'
    first = content.split('.' + veggie, maxsplit=2)[1]
    second = first.split('\n', maxsplit=4)
